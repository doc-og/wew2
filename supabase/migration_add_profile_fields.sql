-- Migration: Add profile fields and set up guardian relationships
...
-- Set up guardian relationships as described:
-- seano is Guardian Owner for roseo
-- amo is guardian for roseo  
...
-- Get user IDs for the relationships
DO $$
DECLARE 
    seano_id UUID;
    roseo_id UUID;
...
BEGIN
    ...
    -- roseo has approved contacts seano and amo
    INSERT INTO approved_contacts (user_id, e164, status, created_at, updated_at) VALUES
    (roseo_id, '+1seano', 'approved', NOW(), NOW()),
    (roseo_id, '+1amo', 'approved', NOW(), NOW())
    ...
    -- seano has approved contacts amo and roseo
    INSERT INTO approved_contacts (user_id, e164, status, created_at, updated_at) VALUES
    (seano_id, '+1amo', 'approved', NOW(), NOW()),
    (seano_id, '+1roseo', 'approved', NOW(), NOW())
    ON CONFLICT (user_id, e164) DO UPDATE SET 
        status = 'approved', 
        updated_at = NOW();
    ...
    -- amo has approved contacts roseo and seano
    INSERT INTO approved_contacts (user_id, e164, status, created_at, updated_at) VALUES
    (amo_id, '+1roseo', 'approved', NOW(), NOW()),
    (amo_id, '+1seano', 'approved', NOW(), NOW())
    ON CONFLICT (user_id, e164) DO UPDATE SET 
        status = 'approved', 
        updated_at = NOW();

    -- Add guardian roles (assuming a roles table exists)
    -- seano is Guardian Owner for roseo
    INSERT INTO roles (user_id, role, target_user_id, created_at) VALUES
    (seano_id, 'guardian_owner', roseo_id, NOW()),
    (amo_id, 'guardian', roseo_id, NOW())
    ON CONFLICT (user_id, role, target_user_id) DO NOTHING;

    RAISE NOTICE 'Guardian relationships set up successfully';
END $$;
